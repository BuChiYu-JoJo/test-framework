...
LOCATOR_SYSTEM_PROMPT = """你是一个专业的 Web 自动化测试工程师，擅长分析网页结构并生成精准的定位符。

你的任务：
1. 分析页面结构文本中的可交互元素
2. 为每个元素生成最优的 CSS Selector
3. 输出一 JSON 对象

输出格式要求：
- key 格式：页面名.元素描述（英文小写下划线，如 login_username、login_password_btn）
- 每个 locator 包含：type（固定css）、value（CSS选择器）、ai_confidence（0.0-1.0）
- 优先 CSS Selector，仅在必要时使用 XPath
- 置信度：id 0.95+，data-testid 0.90+，class 部分匹配 0.6-0.8，纯 tag 0.3-0.5
- 直接输出 JSON，不要任何解释文字"""
...
class AILocatorService:
    ...
    def _analyze_html_content(self, html_text: str, page_id: str) -> Dict[str, Any]:
        ...
        result_text = self.ai.generate(prompt, system_prompt=LOCATOR_SYSTEM_PROMPT)
        locators = self._parse_locator_response(result_text)
    ...
    def generate_with_auth(
        ...
        try:
            ...
            if login_case and base_url:
                ...
                # 用 TestEngine 执行登录用例（保持 page context）
                # 需要从 engine 的父目录导入，使 engine 作为包可正确解析相对导入
                engine_kwargs = {
                    "headless": True,
                    "base_url": base_url,
                    "execution_id": f"ai_loc_{login_case_id}",
                }
                # 动态确保 sys.path 正确
                ...
                # 直接用同一个 Playwright page 完成登录
                result = engine.execute_case_on_page(
                    engine.load_case(case_file.name),
                    page=page,
                )
                logger.info(f"[AILocator] 登录用例执行完成: {result.status}")
