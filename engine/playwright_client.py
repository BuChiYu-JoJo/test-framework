...
class PlaywrightClient:
    ...
    def launch(self) -> "PlaywrightClient":
        ...
        # Windows 上 uvicorn 会将全局事件循环策略改为 WindowsSelectorEventLoopPolicy，
        # 而 SelectorEventLoop 不支持子进程通信，Playwright 启动 Chromium 时会抛 NotImplementedError。
        # 在 launch 期间临时切换为 ProactorEventLoop 策略，启动完成后恢复原策略。
        import sys as _sys
        import asyncio as _asyncio
        _old_policy = None
        if _sys.platform == "win32":
            _old_policy = _asyncio.get_event_loop_policy()
            if not isinstance(_old_policy, _asyncio.WindowsProactorEventLoopPolicy):
        ...
        try:
            ...
            if _sys.platform == "win32" and _old_policy is not None:
