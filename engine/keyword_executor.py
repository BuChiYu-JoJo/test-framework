...
class KeywordExecutor:
    ...
    def __init__(self, locator_resolver=None, base_url: str = None, ai_healer=None, execution_id: str = ""):
        ...
        # Phase 1: AI Locator Healer（可选注入，None 则禁用）
    ...
    def _build_locator_from_strategy(self, page: Page, strategy: Dict):
        ...
        if strategy_type == "aria-label":
            return page.locator(f'[aria-label="{str(value)}"]').first
        if strategy_type == "href":
            return page.locator(f'[href="{str(value)}"]').first
    ...
    def _with_locator(self, target, page: Page, callback):
        ...
        if isinstance(target, dict) and target.get("strategies"):
            ...
            # 所有预定义策略失败后，交给 AI 截图分析自动修复
            if self.ai_healer and self.ai_healer.available:
                target_key = target.get("key", "")
                try:
                    new_selector = self.ai_healer.heal(
                        page, target_key, execution_id=self.execution_id
                    )
                    if new_selector:
                        locator = page.locator(new_selector).first
                        self.last_strategy_match = {
                            "key": target_key,
                            "type": "ai_healed",
                            "value": new_selector,
                    ...
                    print(f"   [AIHealer] Healed selector also failed for {target_key}: {heal_err}")
            # ─────────────────────────────────────────────────────────
    ...
    def wait(self, seconds: float, page: Page) -> None:
        timeout_ms = max(int(float(seconds) * 1000), 0)
    ...
    def wait_for_url(self, pattern: str, page: Page) -> None:
        ...
        wildcard_pattern = str(pattern or "")
        normalized = wildcard_pattern.replace("**", "*")
        if normalized and fnmatch.fnmatch(current_url, normalized):
            return
