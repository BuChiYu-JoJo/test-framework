    ...
    <div v-show="isExpanded" ref="logContainerRef" class="log-container" @scroll="onScroll">
      ...
      >
        ...
        <div class="log-body">
          ...
          <div v-if="toMetaEntries(log.meta).length" class="log-meta">
            <span
              v-for="entry in toMetaEntries(log.meta)"
              :key="entry.key"
              class="meta-pill"
            >
...
const props = defineProps({
  /** @type {{ timestamp?: string, level?: string, message: string, meta?: Record<string, unknown> }[]} */
  logs: {
    type: Array,
    default: () => [],
  },
