...
def scan_locators(
    ...
    """
    失效扫描：逐条校验项目下所有 locator 的命中数，产出失效列表。
    如提供 page_url 则只扫描对应页面的 locators。
    """
    from app.services.locator_verifier import verify_locator_strategy
    ...
    results = []
    for loc in locators:
        strategies = _load_strategies(loc)
        primary = strategies[0] if strategies else {}
        selector = primary.get("value") or loc.selector or ""
        selector_type = primary.get("type") or loc.selector_type or "css"

        if not selector or not page_url:
            results.append({
                "locator_key": f"{loc.page_name}.{loc.element_key}",
                "verdict": "skipped",
                "reason": "无 page_url 或 selector 为空",
            })
            continue

        vr = verify_locator_strategy(
            page_url=page_url,
            selector=selector,
        ...
        results.append({
            ...
            "selector_type": selector_type,
            "verdict": vr.verdict,
            "hit_count": vr.hit_count,
            "error": vr.error,
        })

    failed = [r for r in results if r.get("verdict") in ("miss", "error")]
    return {
        "code": 0,
        "data": {
            "total": len(results),
...
def get_locator_health(project_id: int, db: Session = Depends(get_db)):
    """返回项目内所有 locator 的健康色汇总。"""
    from app.services.locator_health import compute_project_locator_health
    try:
        health_map = compute_project_locator_health(project_id)
        return {"project_id": project_id, "health": health_map}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"health compute failed: {str(e)}")


@router.post("/heal-batch")
def heal_batch_locators(
    project_id: int,
    locator_keys: List[str],
    page_url: Optional[str] = None,
    db: Session = Depends(get_db),
):
    """批量触发 AI 修复（仅将修复任务入队，实际修复由后台执行）。"""
    return {
        "code": 0,
        "data": {
            "queued": locator_keys,
            "message": f"已将 {len(locator_keys)} 个 locator 加入 AI 修复队列",
        },
    }
