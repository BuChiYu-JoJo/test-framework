...
def run_test_case(execution_id: str, case_id: int, project_id: int, env: str, db_url: str):
    ...
    try:
        ...
        try:
            ...
            # 构建 repair_recorder 回调（HTTP POST 到 locator-repair API，不阻塞引擎）
            def _repair_recorder(target_key, old_strategies, new_strategy, verification, execution_id=execution_id):
                import threading, requests, json as _json
                def _post():
                    try:
                        payload = {
                            "project_id": project_id,
                            "locator_key": target_key,
                            "triggered_by": "ui_run",
                            "execution_id": str(execution_id),
                            "old_strategies": old_strategies or [],
                            "new_strategy": new_strategy or {},
                            "hit_count": verification.get("hit_count", 0),
                            "verdict": verification.get("verdict", "error"),
                        }
                        requests.post(
                            "http://127.0.0.1:8000/api/v1/locator-repair/records",
            ...
            # 构建 hit_stats_callback 回调
            def _hit_stats_callback(locator_key, matched_type, matched_value, fallback_depth):
                import threading
                def _record():
                    try:
                        from app.services.locator_health import record_hit_stat
                        record_hit_stat(
                            project_id=project_id,
            ...
            engine_kwargs = {
                ...
                "event_bus": event_bus,
                "repair_recorder": _repair_recorder,
                "hit_stats_callback": _hit_stats_callback,
            }
            if base_url:
            ...
            for step in result.steps:
                exec_step = ExecutionStep(
                    ...
                    matched_strategy_type=step.matched_strategy_type or "",
                    matched_strategy_value=step.matched_strategy_value or "",
                    matched_strategy_priority=step.matched_strategy_priority or 0,
                )
                db.add(exec_step)
    ...
    return [
        {
            ...
            "matched_strategy_type": s.matched_strategy_type or "",
            "matched_strategy_value": s.matched_strategy_value or "",
            "matched_strategy_priority": s.matched_strategy_priority or 0,
        }
        for s in steps
    ]
...
def stream_execution(execution_id: str, db: Session = Depends(get_db)):
    ...
    if record.status not in ("pending", "running"):
        ...
        for s in steps:
            history_steps.append({'type': 'step', 'step_no': s.step_no, 'action': s.action, 'target': s.target or '', 'value': s.value or '', 'status': s.status, 'actual': s.actual or '', 'error_msg': s.error_msg or '', 'duration_ms': s.duration_ms, 'screenshot': s.screenshot or ''})
            history_steps[-1]["matched_strategy_type"] = s.matched_strategy_type or ""
            history_steps[-1]["matched_strategy_value"] = s.matched_strategy_value or ""
            history_steps[-1]["matched_strategy_priority"] = s.matched_strategy_priority or 0
...
from app.services.case_auto_fix import CaseAutoFixService
...
class AutoFixPreviewResponse(BaseModel):
    execution_id: str
    step_no: int
    proposed: bool
    old_target: str = ""
...
@router.post("/auto-fix/preview", response_model=AutoFixPreviewResponse)
def preview_auto_fix_failed_step(
    execution_id: str = Body(...),
    step_no: int = Body(...),
):
    """只生成自动修复建议，不立即回写用例。"""
    service = CaseAutoFixService()
    result = service.preview_failed_step(
        execution_id=execution_id,
        step_no=step_no,
    )
    return AutoFixPreviewResponse(
        execution_id=execution_id,
        step_no=step_no,
        proposed=result.get("proposed", False),
        old_target=result.get("old_target", ""),
...
def auto_fix_failed_step(
    ...
    读取失败步骤的截图 → AI 分析生成真实 selector → 更新用例 → 返回修复结果
    """
    service = CaseAutoFixService()
    result = service.fix_failed_step(
        execution_id=execution_id,
        step_no=step_no,
    )


@router.post("/auto-fix/apply", response_model=AutoFixResponse)
def apply_auto_fix_failed_step(
    execution_id: str = Body(...),
    step_no: int = Body(...),
):
    """确认并应用自动修复建议。"""
...
@router.post("/execute-with-fix")
def execute_with_auto_fix(
    project_id: int,
    case_ids: List[int] = Query(None),
    ...
    流程：
    1. 查找并执行 __precondition__ 前置登录用例（如果有）
    2. 执行目标用例
    3. 失败时：AI 分析截图 → 生成 selector → 更新用例 → 重试（最多 max_auto_fixes 次）
    """
    from app.models.project import Project

    # 1. 查找前置用例
    precondition = db.query(Case).filter(
        Case.project_id == project_id,
        Case.tags.like('%__precondition__%'),
    ).first()

    all_case_ids = []
    if precondition:
        all_case_ids.append(precondition.id)
    ...
    all_case_ids.extend(valid_case_ids)

    if not all_case_ids:
        raise HTTPException(status_code=400, detail="No valid cases to execute")
    ...
    for case_id in all_case_ids:
        ...
        if background_tasks:
            background_tasks.add_task(
                ...
                is_precondition=(case_id == precondition.id if precondition else False),
            )
        execution_ids.append(exec_id)
    ...
    return {
        ...
        "precondition_executed": bool(precondition),
    }


def run_test_case_with_fix(
    execution_id: str,
    case_id: int,
    project_id: int,
    env: str,
    ...
    try:
        ...
        try:
            ...
            if project and project.env_config:
                try:
                    env_config = json.loads(project.env_config) if isinstance(project.env_config, str) else project.env_config
            ...
            auto_fix_service = CaseAutoFixService() if not is_precondition else None
            fix_count = 0

            for attempt in range(max_auto_fixes + 1):
                ...
                if result.status.value == "passed":
                    exec_record.status = "passed"
                    exec_record.result = "passed"
                    break

                if is_precondition:
                    exec_record.status = "failed"
                    exec_record.error_msg = result.error_msg or "Precondition failed"
                    break

                failed_step = next(
                    (s for s in result.steps if s.status.value == "failed"), None
                )
                if not failed_step:
                    exec_record.status = "failed"
                    exec_record.error_msg = result.error_msg or "Unknown error"
                    break

                if fix_count >= max_auto_fixes:
                    exec_record.status = "failed"
                    exec_record.error_msg = f"Auto-fix exhausted after {fix_count} fixes: {failed_step.error}"
                    break

                step_screenshot = failed_step.screenshot if failed_step else None

                if auto_fix_service and step_screenshot:
                    fix_result = auto_fix_service.fix_failed_step(
                        execution_id=execution_id,
                        step_no=failed_step.step_no,
                    )
                    if fix_result.get("fixed"):
                        fix_count += 1
                        case_record = db.query(Case).filter(Case.id == case_id).first()
                        with open(case_file.name, "w", encoding="utf-8") as f:
                            f.write(case_record.content)
                        print(f"[AutoFix] Retry {fix_count}: fixed step {failed_step.step_no}")
                        continue

                exec_record.status = "failed"
                exec_record.error_msg = f"Step {failed_step.step_no} failed after {fix_count} fixes: {failed_step.error}"
                break

            exec_record.duration_ms = int(result.duration_ms or 0)
            exec_record.finished_at = datetime.now()
            if result.screenshots:
                exec_record.screenshots = json.dumps(result.screenshots or [])

        except Exception as e:
            print(f"[AutoFix] Execution error: {e}")
            exec_record.status = "failed"
