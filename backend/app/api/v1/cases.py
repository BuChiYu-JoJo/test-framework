...
def import_cases(
    ...
    from engine.parser.excel_parser import ExcelParser
...
def batch_create_cases(
    ...
    """
    批量入库用例草稿（来自 PRD 解析）。
    drafts 中每条需含 title / case_id / module / priority / content 字段。
    """
    ...
    for draft in drafts:
        case_id_str = draft.get("case_id") or draft.get("draft_id") or ""
        title = draft.get("title") or draft.get("name") or "未命名"

        if not case_id_str:
            skipped.append({"draft": draft, "reason": "缺少 case_id"})
            continue

        existing = db.query(Case).filter(
            Case.project_id == project_id,
            Case.case_id == case_id_str,
        ).first()
        if existing:
            skipped.append({"draft_id": case_id_str, "reason": "case_id 已存在"})
            continue
        ...
        case = Case(
            ...
            case_id=case_id_str,
            project_id=project_id,
            module=draft.get("module", "default"),
            priority=draft.get("priority", "P2"),
            tags=json.dumps(draft.get("tags") or []),
            content=draft.get("content") or draft.get("yaml_content") or "",
        )
        db.add(case)
        db.commit()
        db.refresh(case)
...
@router.post("/{case_id}/self-check")
def self_check_case(case_id: int, db: Session = Depends(get_db)):
    """
    单用例 AI 自检：干跑 + AI 验证，返回失败步骤与可疑断言。
    当前版本返回用例内容摘要供前端展示，完整 AI 验证由后续版本实现。
    """
    case = db.query(Case).filter(Case.id == case_id).first()
    if not case:
        raise HTTPException(status_code=404, detail="用例不存在")
