...
def init_db():
    """初始化数据库（创建所有表）"""
    from app.models import case, project, execution, locator, scheduler, api_case
    from app.models import task, locator_repair_record, locator_hit_stats
    Base.metadata.create_all(bind=engine)
    _ensure_locator_columns()
    _ensure_execution_step_columns()
    _ensure_api_test_task_columns()
    _ensure_locator_health_columns()
    _ensure_case_automation_columns()
...
def _ensure_api_test_task_columns():
    """Add API test task result columns for existing SQLite databases."""
    inspector = inspect(engine)
    if "api_test_tasks" not in inspector.get_table_names():
        return

    columns = {column["name"] for column in inspector.get_columns("api_test_tasks")}
    alter_statements = []

    if "result_json" not in columns:
        alter_statements.append(
            "ALTER TABLE api_test_tasks ADD COLUMN result_json TEXT DEFAULT '[]'"
        )
...
def _ensure_locator_health_columns():
    """Add health_color and last_healed_at columns to locators table."""
    inspector = inspect(engine)
    if "locators" not in inspector.get_table_names():
        return
    ...
    if "health_color" not in columns:
        alter_statements.append(
            "ALTER TABLE locators ADD COLUMN health_color VARCHAR(8) DEFAULT 'grey'"
        )
    if "last_healed_at" not in columns:
        alter_statements.append(
            "ALTER TABLE locators ADD COLUMN last_healed_at DATETIME"
        )
...
def _ensure_case_automation_columns():
    """Add tags_json and automation columns to cases table."""
    inspector = inspect(engine)
    ...
    if "tags_json" not in columns:
        alter_statements.append(
            "ALTER TABLE cases ADD COLUMN tags_json TEXT DEFAULT '[]'"
        )
    if "automation" not in columns:
        alter_statements.append(
            "ALTER TABLE cases ADD COLUMN automation INTEGER DEFAULT 0"
        )
