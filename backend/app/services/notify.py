...
class FeishuNotifier:
    ...
    def send(self, msg: str, msg_type: str = "text") -> bool:
        ...
        """
        if not self.enabled:
            self._safe_console_print(f"[FeishuNotifier] Webhook 未配置，跳过通知: {msg[:50]}")
            return False
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=10) as resp:
                ...
                if result.get("code") == 0 or result.get("StatusCode") == 0:
                    self._safe_console_print("[FeishuNotifier] 发送成功")
                    return True
                else:
                    self._safe_console_print(f"[FeishuNotifier] 发送失败: {result}")
                    return False
        except Exception as e:
            self._safe_console_print(f"[FeishuNotifier] 发送异常: {e}")
            return False
