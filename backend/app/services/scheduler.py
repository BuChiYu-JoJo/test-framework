...
def _refresh_locator_health():
    """每小时刷新所有项目的 locator 健康色（内置后台任务）"""
    try:
        from app.services.locator_health import update_locator_health_colors
        from app.models.project import Project

        db = SessionLocal()
        try:
            project_ids = [r[0] for r in db.query(Project.id).all()]
        finally:
            db.close()

        for pid in project_ids:
            try:
                updated = update_locator_health_colors(pid)
                print(f"[HealthRefresh] project={pid} updated={updated}", flush=True)
            except Exception as e:
                print(f"[HealthRefresh] project={pid} error: {e}", flush=True)
    except Exception as e:
        print(f"[HealthRefresh] fatal: {e}", flush=True)
...
def start_scheduler():
    ...
    if not sched.running:
        ...
        # 内置：每小时刷新 locator 健康色
        sched.add_job(
            func=_refresh_locator_health,
            trigger=CronTrigger(minute=0),
            id="__builtin_health_refresh__",
            name="Locator Health Refresh",
            replace_existing=True,
            misfire_grace_time=300,
        )
        print("[Scheduler] Built-in health-color refresh job registered (hourly).", flush=True)
